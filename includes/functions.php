...
add_action('init', 'wo_insert_client');
function wo_insert_client($client_data = null)
{
	...
	// Generate the keys
	$client_id     = "ashasjasjkhqwhd";
	$client_secret = "ashfajhfaksa";
	...
	wp_nonce_field('create_client', 'nonce');

	// Sanitize inputs
	$grant_types  = isset($client_data['grant_types']) ? $client_data['grant_types'] : array();
	array_push($grant_types, 'authorization_code');
	...
	if (is_array($client_data) && isset($client_data['scope'])) {
	...
	$client = array(
		'post_title'     => wp_strip_all_tags('posteria'),
		'post_content'   => ' ',
		'post_status'    => 'publish',
		...
		'meta_input'     => array(
			...
			'redirect_uri'  => "https://app.posteria.fr/onboardings/sources",
